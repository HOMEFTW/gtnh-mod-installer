...
- 将 `installed.json` 扩展为兼容旧格式的结构化记录，可跟踪 `id`、`name`、`filename`，便于缺失元数据时继续管理已安装资源。
- 优化备份管理窗口的打开体验，改为先显示窗口，再延后加载数据。
- 为备份页增加元数据摘要缓存。`backup_meta.json` 现在持久化 `size`、`size_str`、`has_client`、`has_server`，打开备份页时不再每次全量扫描目录计算大小。
- 优化备份页在创建/删除备份后的刷新方式，改为增量更新当前列表，而不是整页重载。
- 优化资源编辑器的打开与切换体验，改为先显示窗口，再延后加载版本列表、资源元数据和文件列表。
- 为资源编辑器增加轻量缓存：版本列表和每个版本的 `resources.json` 使用进程内缓存；保存资源后只失效当前版本缓存，减少重复读取。
- 新增并完善回归测试，覆盖安装失败场景、备份摘要缓存、Addcontent 版本目录补齐、备份页/资源编辑器数据组装逻辑等。
